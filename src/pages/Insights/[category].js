...
export default function CategoryPage({ blog_list, category }) {
    ...
    <>
      ...
      <div className={styles.mainContainerBlogs}>
        <div className={styles.pageTitle}>
          <h3>Articles with our opinions, views and reports summaries on <strong>{category} </strong></h3>
        </div>

        <div className={styles.blogCategoriesCont}>
          {arrayBlogs.map((blog, index) => (
            <div className={styles.blogCategoryBox} key={index}>
              <Link href={`/Insights/${category}/${blog.contentSlug}`}>
